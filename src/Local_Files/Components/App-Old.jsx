...
class App extends React.Component {
	...
	constructor(props) {
		...
		if (window.localStorage.getItem("QuizState") === null) {
			...
			if (window.localStorage.getItem("LastVisited") + (1000*60*60*6) > Date.now() && import.meta.env.VITE_Version === window.localStorage.getItem("Version")) {
				console.log(JSON.parse(window.localStorage.getItem("QuizState")))
				console.log(window.localStorage.getItem("CompletedQuiz"));
				//this.state.QuestionList = JSON.parse(window.localStorage.getItem("QuizState"));
				if (window.localStorage.getItem("CompletedQuiz")) {
					this.state.ProgramState = "LeaderboardScreen";
					this.state.PreviousState = "LeaderboardScreen";
				}
				else {
	...
	render() {
		...
		switch (this.state.ProgramState) {
			...
			case "LeaderboardScreen":
				...
				programbody = <LeaderboardScreen SwitchProgramState={(State, ShouldMatch) => {this.SwitchProgramState(State, ShouldMatch);}}/>
				break;
	...
	HandleQuestionComplete(Score) {
		...
		setTimeout(() => {
			...
			if (QuestionsCompleted === this.state.QuestionList.length && QuestionsCompleted !== 0 ? true : false) {
				this.SwitchProgramState("LeaderboardScreen", true);
	...
	Save() {
		window.localStorage.setItem("QuizState" , JSON.stringify(this.state.QuestionList));
	}
	// runs when the program is ready to run
	componentDidMount() {
	
		// saves the program
		const SaveTimer = setInterval(() => {this.Save(); this.setState({QuestionsCompleted : window.localStorage.getItem("QuestionsCompleted")})}, 1*1000);
