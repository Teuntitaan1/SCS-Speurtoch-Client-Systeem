...
export default function DoneQuestionsScreen(props) {
    ...
    const QuizState = JSON.parse(window.localStorage.getItem("QuizState"));
    ...
    return(
        <>
            ...
            <div style={{marginTop : -5+"%", opacity : ActiveQuestion !== null ? 1 : 0, transition : 'opacity 500ms ease-in-out'}}>
                ...
                {ActiveQuestion !== null ?
                    <>
                        <h2 style={{textAlign : 'center'}}>{QuizState[ActiveQuestion].Description}</h2>
                        <div style={{display : 'flex', flexWrap : 'wrap', justifyContent : 'center'}}>
                            {QuizState[ActiveQuestion].Options.map((Option, index) => 
                                <div key={index} style={
                                    {
                                        backgroundColor : Option === QuizState[ActiveQuestion].CorrectAnswer ? "#7ed957" : "#e14747",
                                        width : 2.5+"rem",
                                        height : 2.5+"rem",
                                        borderRadius : 15+"px",
                                        textAlign : 'center'
                        ...
                        <p><span style={{fontStyle : 'italic', fontWeight : 'bold'}}>{QuizState[ActiveQuestion].CorrectAnswer}</span> was het goede antwoord!</p>
                        
                        <div style={{marginTop : 0+"vh"}}>
                            <h2 style={{textAlign : 'center'}}>Wist je dat?</h2>
                            <p>{QuizState[ActiveQuestion].ExtraInfo}</p>
                        </div>
                    </>
                    : null
                }
