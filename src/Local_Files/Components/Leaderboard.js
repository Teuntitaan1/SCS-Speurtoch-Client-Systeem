    ...
    return(
        ...
        <table border={1} cellPadding={5} style={{marginTop : 1+"rem", width  : 100+"%", borderCollapse : 'collapse', borderRadius : 1+"rem"}}>
            <tbody>
                ...
                {this.props.Leaderboard.map((Entry) => {if(props.Uuid === Entry.Uuid) {return true;} return false;}).includes(true) === true && props.Leaderboard.slice(0, 5).map((Entry) => {if(props.Uuid === Entry.Uuid) {return true;} return false;}).includes(true) !== true ?
